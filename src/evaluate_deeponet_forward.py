"""
Evaluate the trained DeepONet forward-operator baseline.

This script evaluates the saved model on full velocity fields, not just
random point samples.

Run:
    python src/evaluate_deeponet_forward.py
"""
...
def build_branch_features(data) -> np.ndarray:
    """Build branch input features. Must match train_deeponet_forward.py."""
...
def train_val_split(
    ...
    """Create the same train/validation split used in training."""
...
def evaluate_validation_set(
    ...
    """Evaluate full-field relative L2 error over validation samples."""
    ...
    return np.array(errors), predictions, val_indices
...
def plot_best_median_worst_examples(
    ...
    for row, (label, error_position) in enumerate(selected):
        ...
        error_vmax = float(abs_error.max())
        ...
        for col, (panel_title, field, vmin, vmax_panel) in enumerate(panels):
            ax = axes[row, col]
...
def main() -> None:
    ...
    errors, predictions, evaluated_indices = evaluate_validation_set(
        model=model,
        data=data,
