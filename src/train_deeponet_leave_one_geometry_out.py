"""
Leave-one-geometry-out DeepONet generalization experiment.
...
Smoke test:
    python src/train_deeponet_leave_one_geometry_out.py --held-out-geometry sinusoidal --epochs 20

Main run:
    python src/train_deeponet_leave_one_geometry_out.py --held-out-geometry sinusoidal --epochs 1000 --batch-size 8192 --learning-rate 5e-4
...
def sample_training_batch(
    ...
    """Sample a random pointwise training/evaluation batch."""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Options: unconstrained,power_law_aware,geometry_aware"
        ),
    )
...
def main() -> None:
    ...
    per_sample_path = Path(
        f"results/leave_one_geometry_out_{safe_geometry}_per_sample.csv"
    )
    summary_path = Path(
        f"results/leave_one_geometry_out_{safe_geometry}_summary.csv"
    )
    ...
    plot_summary(
        ...
        output_path=Path(
            f"figures/leave_one_geometry_out_{safe_geometry}_summary.png"
        ),
    )
    ...
    plot_by_fluid(
        ...
        held_out_geometry=args.held_out_geometry,
        output_path=Path(
            f"figures/leave_one_geometry_out_{safe_geometry}_by_fluid.png"
        ),
    )
